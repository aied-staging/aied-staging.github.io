...
jQuery(function ($) {
   ...
   ====================================== */
    ...
    $("#team-three-slider-3").owlCarousel({
        items: 4,
        dots: false,
        nav: false,
        responsive: {
...
function filterSelection(c) {
  ...
  for (i = 0; i < x.length; i++) {
    ...
    w3RemoveClass(x[i], "blog-list-item-show");
    
    if (x[i].getAttribute("type").indexOf(c) > -1) w3AddClass(x[i], "blog-list-item-show");
  }
}
...
// researchSelection("all")
showAllPeople()
function peopleSelection(c) {
    showAllPeople();
    $( ".team-box").not("[type*="+c+"]").css("opacity", .1)
}
